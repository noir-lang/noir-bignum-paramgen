 ...
 */
fn split_into_120_bit_limbs(_input: &BigUint) -> Vec<BigUint> {
 ...
 *          (either 2 or 4)
 */
fn split_into_60_bit_limbs<const NUM_CHUNKS: usize>(_input: &BigUint) -> [Vec<BigUint>; NUM_CHUNKS] {
    let num_bits = _input.bits();
    let num_limbs: usize = (num_bits / 120) + (num_bits % 120 != 0) as usize;
     let mut input = _input.clone();
 ...
 */
fn compute_bn_instance_parameters(modulus: &BigUint) -> BNInstance {
 ...
 */
fn bn_runtime_instance_from_string(
    modulus_str: String,
    name: String
) -> String {
